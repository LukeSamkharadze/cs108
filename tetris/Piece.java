...
public class Piece {
  ...
  public Piece(TPoint[] points) {
    ...
    for (int i = 0; i < points.length; i++) this.getBody()[i] = new TPoint(points[i]);

    this.height = getMaxHeight();
    this.width = getMaxWidth();
    this.skirt = _getSkirt();
  }

  private int[] _getSkirt() {
    int[] skirt = new int[width];
    Arrays.fill(skirt, this.height-1);
    for (var tPoint : this.body) skirt[tPoint.x] = Math.min(tPoint.y, skirt[tPoint.x]);
    return skirt;
  }

  private int getMaxHeight() {
    int maxHeight = 0;
    for (var tPoint : this.body) maxHeight = Math.max(tPoint.y + 1, maxHeight);
    return maxHeight;
  }

  private int getMaxWidth() {
    int maxWidth = 0;
    for (var tPoint : this.body) maxWidth = Math.max(tPoint.x + 1, maxWidth);
    return maxWidth;
  }
  ...
  public Piece computeNextRotation() {
    var newPoints = new TPoint[this.body.length];

    for (int i = 0; i < this.body.length; i++)
      newPoints[i] = new TPoint(this.body[i].y, this.width - 1 - this.body[i].x);
  ...
  public boolean equals(Object obj) {
    ...
    for (var tPoint : this.body)
      if (Arrays.stream(other.body).noneMatch(tPoint::equals))
        return false;
  ...
  public static Piece[] getPieces() {
    ...
    if (Piece.pieces == null) Piece.pieces = new Piece[]{
